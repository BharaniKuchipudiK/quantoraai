...
| **Open a repository in the desk** | Connected user | `POST /api/github/checkout` |
| **List your repositories / branches** | Connected user | `POST /api/github/list-repos`, `/api/github/list-branches` |
...
## Opening a repository in the desk

`POST /api/github/checkout` is a different thing from `/api/github/preview`,
and both are kept.

Preview reads **ten files**, ranked by how well their names match the user's
sentence, and hands them to the model as an attachment. It is a reading sample
and it was always honest about being one — right for a question about a
repository nobody intends to change.

Checkout answers the other question: *give me the project.* Every editable text
file up to a stated budget — 400 files, 6MB total, 512KB per file — at a named
commit.

**The commit SHA is the point.** A checkout without one is a pile of files;
with one it is a working copy. The desk knows what it started from, a diff
means something, and a push can refuse when the branch has moved underneath it.
It is resolved before the tree is read, and the checkout fails rather than
returning files it cannot anchor.

**Truncation is reported, never silent.** A partial tree that looks complete is
worse than a refusal, because the model reasons about a codebase with holes and
cannot tell. `omitted` carries a count and a human reason per category, and
`notice` renders one sentence the caller must show. The categories:

- generated directories (`node_modules`, `dist`, `coverage`, …)
- lockfiles — larger than every source file combined, and regenerated by
  installing dependencies; nothing regenerates the source dropped to fit one
- files that are not text this desk can edit
- files over the per-file cap, dropped rather than truncated: half a file is
  worse than none, because the model cannot see where it was cut
- anything beyond the budget

When GitHub itself truncates the tree, the notice says the checkout is partial
and that unseen files are **present but unloaded** — so the model does not
conclude a file is absent.

Opening a repository replaces the desk. It asks first when the desk has files,
and it refuses to replace them with an empty result: a checkout that read
nothing is a failure, not an instruction to wipe what the user had.
