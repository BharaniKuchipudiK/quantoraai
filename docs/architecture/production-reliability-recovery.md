...
- The next exact-commit canary rendered the calculator but caught a generated website reading `localStorage` inside the opaque-origin iframe despite the prompt contract. Completed build responses now pass a deterministic executable-artifact contract before a route is committed, so invalid output can use the already-budgeted independent fallback without poisoning shared operational circuits; the compiler rejects unsupported storage as defense in depth, and iframe `rendered` is emitted only after the application mounts content rather than when the harness merely loads.
- The first production canary on commit `0430090a15ba960845cfab345c5d0b3aef9755c0` then proved a narrower executable-contract gap: the website VFS had all required files and selectors and compiled successfully, but its entry module imported `App` without mounting it, leaving `#root` empty. Golden build responses must now prove an explicit React root lookup, root creation, and component render before a provider route is committed; empty-root and thrown-runtime failures have distinct iframe trace codes.
- The follow-up Preview canary caught the symmetric interaction gap: a calculator rendered the required controls but clicking `1` did not update the display. The calculator output contract now requires a state-backed display and a click handler wired to its setter before route commitment; the browser remains the final behavioral proof.
...
| Response → artifact/VFS | The current response must contain runnable fenced files/code; golden React VFS output must explicitly mount a component into `#root`; stale VFS alone is not success. | Missing files, forbidden sandbox APIs, missing root mount, or an absent `artifact.vfs` boundary prevent the route from being committed as a successful build. |
