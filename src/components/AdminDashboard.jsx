    ...
    <div style={{
      ...
      <div style={{ marginBottom: '32px' }}>
        <h1 style={{ fontSize: '2.2rem', fontWeight: '800', margin: '0 0 8px 0', letterSpacing: '-0.02em', background: 'linear-gradient(135deg, #f8fafc 0%, #94a3b8 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
          Quantora Performance Analytics
        </h1>
        <p style={{ color: '#64748b', margin: 0, fontSize: '0.95rem' }}>Global Telemetry & Infrastructure Health Monitoring</p>
      </div>

      {/* Top KPIs Row - Dense Grid */}
      <div style={{
        display: 'grid',
        ...
        <MiniKpi title="Active Users" value={metrics.activeConnections} sparklineColor="#10b981" icon={<Users size={16}/>} trend="+12%" />
        ...
        <MiniKpi title="Global Avg Latency" value={`${metrics.avgLatency}ms`} sparklineColor="#f59e0b" icon={<Clock size={16}/>} trend="-14ms" trendGood={true} />
        ...
        <MiniKpi title="Compute Hours" value={`${metrics.computeHours.toFixed(1)}h`} sparklineColor="#ec4899" icon={<Cpu size={16}/>} trend="+12h" />
      </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #1f2937', color: '#64748b', textAlign: 'left' }}>
                  ...
                  <th style={{ padding: '0 0 12px 0', fontWeight: '500' }}>Latency</th>
                  <th style={{ padding: '0 0 12px 0', fontWeight: '500' }}>Load</th>
                </tr>
              </thead>
              <tbody>
                {metrics.endpoints.map((ep, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)' }}>
                    <td style={{ padding: '16px 0', color: '#f1f5f9', fontWeight: '500' }}>{ep.name}</td>
                    ...
                    <td style={{ padding: '16px 0', color: '#cbd5e1', fontFamily: 'monospace' }}>{ep.latency}ms</td>
                    <td style={{ padding: '16px 0' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <span style={{ fontSize: '0.75rem', color: '#94a3b8', width: '30px' }}>{ep.load}%</span>
                      </div>
                    </td>
                  </tr>
                ))}
              ...
              {metrics.activeSessions.map((s, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '8px' }}>
                  <div>
                    ...
                    <div style={{ color: '#64748b', fontSize: '0.75rem', marginTop: '4px' }}>{s.model} • {s.id}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ color: '#cbd5e1', fontSize: '0.85rem', fontFamily: 'monospace' }}>{s.tokens} tks</div>
                    <div style={{ color: '#64748b', fontSize: '0.75rem', marginTop: '4px' }}>{s.duration}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="panel" style={{ background: '#09090b', border: '1px solid #1f2937', borderRadius: '12px', padding: '24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div>
                ...
                <div style={{ width: '100%', height: '6px', background: '#1f2937', borderRadius: '3px', overflow: 'hidden' }}>
                  <div style={{ width: `${metrics.memoryUsage}%`, height: '100%', background: '#8b5cf6', transition: 'width 1s' }}></div>
                </div>
              </div>
            </div>
          </div>
