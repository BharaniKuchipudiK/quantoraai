...
function splitLabel(value = '', max = 22) {
  ...
  for (const word of words) {
    const firstCandidate = `${first} ${word}`.trim();
    if (!first || firstCandidate.length <= max) {
      first = firstCandidate;
      continue;
    }
    const secondCandidate = `${second} ${word}`.trim();
    if (!second || secondCandidate.length <= max) {
      second = secondCandidate;
      continue;
    }
    second = `${second.slice(0, Math.max(0, max - 1)).trim()}…`;
    break;
  }
...
export function StudyFractionBar({ fraction, x, y, width = 116, isLight = false }) {
  ...
  if (!Number.isInteger(denominator) || denominator < 1 || denominator > 12) return null;
  if (!Number.isInteger(numerator) || numerator < 0 || numerator > denominator) return null;
  ...
  return (
    ...
    >
      ...
      {beforeLines.map((line, index) => (
        <text key={`before-${index}-${line}`} x="78" y={68 + (index * 16)} textAnchor="middle" fill={ink} fontSize="12" fontWeight="700">{line}</text>
      ))}
      ...
      {afterLines.map((line, index) => (
        <text key={`after-${index}-${line}`} x="282" y={68 + (index * 16)} textAnchor="middle" fill={ink} fontSize="12" fontWeight="700">{line}</text>
      ))}
