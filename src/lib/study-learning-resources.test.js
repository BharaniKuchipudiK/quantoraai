...
test('lesson, quiz, flashcards, and notes asks stay tutor-like and fail-closed on fake videos', () => {
  ...
  assert.match(studyIcebreakerAsk("Newton's laws"), /apple-tree|ice-puck|force-arrows|book-table/);
  assert.match(studyIcebreakerAsk('Algebra'), /mystery-box|balance-scale/);
  assert.doesNotMatch(studyIcebreakerAsk('Algebra'), /kind="apple-tree"/);
  assert.doesNotMatch(studyLessonAsk('Algebra'), /all three laws/);
