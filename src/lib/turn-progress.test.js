...
test('every line is backed by something observed', () => {
  assert.match(describeTurnPhase({ elapsedSec: 3, modelName: 'Gemini 3 Flash' }).line, /Reaching Gemini 3 Flash/);
...
test('silence is named, not hidden', () => {
  const quiet = describeTurnPhase({ elapsedSec: 25, modelName: 'Nemotron' });
  ...
  const stalled = describeTurnPhase({ elapsedSec: 70, modelName: 'Nemotron', budgetSec: 175 });
...
test('a stalled turn offers something to do, not sympathy', () => {
  ...
  assert.equal(describeTurnPhase({ elapsedSec: SILENT_STALL_SEC - 1 }).stalled, false);
});
