...
export const JOURNEYS = Object.freeze([
  ...
  journey({
    ...
    entry: 'vercel.json',
    gates: { deterministic: ['api/_lib/turn-budget-fits-function.test.ts'] },
    note: 'chat-handler plans a turn of TOTAL_CHAT_BUDGET_MS and spends it across engine attempts, fallbacks, and a final catch that records what happened. The ceiling on that time lives in vercel.json and nothing connected the two: api/chat.ts was the ONLY long-running function in that file with no maxDuration, while generate-office was pinned to 300s and pipeline to 180s. The failure mode is not a slow turn, it is a SILENT one. Seen in production 2026-09-08: an engine failed at +114.7s and the trace ended there, with nothing after it -- chat-handler\'s catch writes an api.chat failed row on any throw and never ran, because the process was gone. The turn the user watched fail left no record that it failed, invisible to the failure digest and to its own reference lookup, and was reported to the person as the connection dying. A budget larger than its own ceiling cannot be observed from inside the process it kills; only a check outside both can see it. The gate requires 30s of margin rather than a bare fit, because the terminal trace is written after the engine work and is exactly the part that matters when a turn overruns.',
  }),
  journey({
