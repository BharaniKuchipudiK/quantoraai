...
/**
 * Take the pending Tools-menu choice, if any, and clear it. The turn planner
 * (Phase 7) asks for this BEFORE it asks a model: a kind the person picked by
 * hand is not a guess to be second-guessed.
 */
export function consumeOfficeToolSelection() {
  const chosen = pendingOfficeTool;
...
export function detectOfficeIntent({ selectedTool = null, messages = [] } = {}) {
  ...
  const window = userTurns.slice(-OFFICE_INTENT_LOOKBACK).reverse();
  for (const turn of window) {
    /*
     * A PLANNED TURN DECIDES (Phase 7). The turn planner read the whole brief
     * and named its lane; that decision rides on the message. The desk used
     * to re-read the words here and show a spreadsheet pane for a web page
     * that only borrowed the word "excel". A planned Office turn names its
     * kind; a planned build, advisor or chat turn is not Office and cancels
     * any older Office intent behind it.
     */
    if (turn.lanePlan && typeof turn.lanePlan === 'object' && turn.lanePlan.lane) {
      return turn.lanePlan.lane === 'office' ? (officeKindFromTool(turn.lanePlan.officeKind) || null) : null;
    }
