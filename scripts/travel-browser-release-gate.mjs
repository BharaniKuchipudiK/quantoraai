...
await page.addInitScript(() => {
  // Reproduce the production regression: a generic preference must never be
  // allowed to suppress the specialist Travel landing experience.
...
await page.route('**/api/**', async (route) => {
  ...
  // Keep unrelated authenticated Studio boot requests deterministic. The
  // release gate is testing browser behaviour, not external services.
  return route.fulfill({
    status: 200,
    contentType: 'application/json',
...
try {
  ...
  // Session restoration is asynchronous. Once authenticated, Studio must be
  // reachable without a real OAuth interaction in this synthetic journey.
  ...
  // Regression gate 1: specialist welcome must exist even when the generic
  // "hide welcome" preference is true.
  ...
  // Regression gate 2: specialist mode owns the experience. Infrastructure
  // chrome must not leak into the Travel workspace.
  ...
  // Regression gate 3: a real user turn must complete within the browser SLA
  // and render the interactive decision card.
  ...
  // Regression gate 4: after a choice is submitted the large card must collapse
  // into compact conversation history: question + selected answer only.
