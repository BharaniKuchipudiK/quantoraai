...
test('the evidence vocabulary has no representation, view, or preference kind', () => {
  ...
  const verified = admission.slice(admission.indexOf('const VERIFIED_KINDS'), admission.indexOf('const ASSESSMENT_BACKED_KINDS'));
...
test('a representation or view kind cannot be normalized into the mastery ledger', () => {
  const forged = normalizeStudyMasteryEvidenceEvent({
    id: 'study.representation.view', conceptId: 'concept-1', kind: 'representation_view', correct: true, score: 1,
    independent: true, observedAt: '2026-09-04T00:00:00.000Z', provenance: 'quantora_authored', sourceRef: 'quantora:study-picture',
  });
...
test('viewing a picture or clicking got-it cannot enter admitted mastery', () => {
  ...
  assert.deepEqual(evaluateStudyEvidenceAdmission(viewed), { admitted: false, reasonCode: 'unverified_evidence_kind' });
...
test('representation coverage telemetry is not an evidence event', () => {
  assert.equal(evaluateStudyEvidenceAdmission({
    id: 'representation_coverage', conceptId: 'concept-1', kind: 'representation_coverage' as StudyMasteryEvidenceEvent['kind'],
    correct: true, score: 1, hintsUsed: 0, independent: true, misconceptionSignal: false,
    provenance: 'quantora_authored', observedAt: '2026-09-04T00:00:00.000Z',
  }).admitted, false);
});

test('planning or showing a representation does not change the learner model', () => {
  const empty: StudyMasteryEvidenceEvent[] = [];
  const before = buildStudyLearnerModel({ conceptId: 'concept-1', conceptKey: 'physics.electricity.emf-terminal-voltage', evidence: empty, estimate: estimateStudyMastery(empty) });
  const plan = planStudyTeachingRepresentation({ message: 'Teach me using images', contextText: 'EMF versus terminal voltage in a battery circuit' });
  ...
  const after = buildStudyLearnerModel({ conceptId: 'concept-1', conceptKey: 'physics.electricity.emf-terminal-voltage', evidence: afterViewing, estimate: estimateStudyMastery(afterViewing) });
