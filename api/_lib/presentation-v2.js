...
export function validatePresentationSpec(input = {}, { consultingGate = 'hard' } = {}) {
  ...
  spec.slides.forEach((slide, index) => {
    requireSemanticPayload(slide, index, consultingGate === 'soft' ? consultingIssues : issues);
  ...
  for (let i = 1; i < spec.slides.length; i += 1) {
    ...
    if (repeated > 2 && !['status_dashboard'].includes(spec.slides[i].type)) {
      consultingIssues.push(`Slides ${i - 1}–${i + 1} repeat the same ${spec.slides[i].type} composition. Use a more purposeful visual grammar.`);
      break;
    }
  }
  ...
  if (spec.slides.length >= 7) {
    ...
    if (bodyTypes.size < 3 && spec.archetype !== 'project_status') {
      consultingIssues.push('Professional decks of seven or more slides must use at least three distinct body composition types.');
    }
  }

  const types = new Set(spec.slides.map((slide) => slide.type));
  if (spec.archetype === 'business_case' && spec.slides.length >= 5) {
    if (!types.has('executive_summary')) consultingIssues.push('Business case requires an executive_summary composition.');
    if (!types.has('comparison') && !types.has('financial_case')) consultingIssues.push('Business case requires an options comparison or financial/value case.');
  ...
  if (spec.archetype === 'qbr' && spec.slides.length >= 5) {
    if (!types.has('executive_summary') && !types.has('status_dashboard')) consultingIssues.push('QBR requires an executive summary or status dashboard.');
    if (!types.has('chart_insight') && !types.has('kpi_strip')) consultingIssues.push('QBR requires KPI/trend evidence, not narrative-only slides.');
  }
  if (spec.archetype === 'project_status' && spec.slides.length >= 4) {
    if (!types.has('status_dashboard')) consultingIssues.push('Project status deck requires a status_dashboard composition.');
  ...
  if (consultingGate === 'soft') warnings.push(...consultingIssues);
  else issues.push(...consultingIssues);
