...
import { getRuntimeGovernorMetrics } from "../runtime-governor-metrics.js";
...
export default async function handler(req: any, res: any) {
  ...
  const [growth, series, suggestionAcceptance, turnPlanRows, failureRead, usageRead, governor] = await Promise.all([
    getGrowthSummary(),
    getDailySeries(14),
  ...
  return res.status(200).json({
    ...
    /* Runtime Governor operational truth, measured from persisted lifecycle
     * events. This is intentionally a reliability panel, not a synthetic
     * infrastructure gauge: it shows whether governed work completes,
     * recovers, stalls, or remains unresolved against outcome evidence. */
    governor,
