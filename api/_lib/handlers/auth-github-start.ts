...
export default async function handler(req: any, res: any) {
  ...
  if (!clientId) {
    return res.redirect(302, `${oauthOrigin(req)}/?auth=error&message=${encodeURIComponent("GitHub sign-in is not configured on this deployment.")}`);
  }

  const state = randomBytes(16).toString("hex");
  const callback = `${oauthOrigin(req)}/api/auth/github/callback`;
