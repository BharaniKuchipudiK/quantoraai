...
// Inject abortControllerRef
code = code.replace(
  'export function useChatStream({',
  'export function useChatStream({\n  abortControllerRef = { current: null },'
);

// We need to pass abortControllerRef from AiStudio.jsx, or create it here.
// Since useChatStream is a hook, we can just declare it:
code = code.replace(
  'export function useChatStream({',
  'export function useChatStream({\n'
);
// Actually it's better to just use useRef inside:
code = code.replace(
  /export function useChatStream\(\{[^\}]+\}\) \{/,
